...
var currentQuestion = urlChunks[urlChunks.length - 1].split('.');
	  ...
	  if(currentQuestion[0] == "QC_001a" || currentQuestion[0] == "QC_001b")
	  {
		 uploader = $('<input type="file" accept="image/*" capture="environment"/>')
	  }
	  else
	  {
		  uploader = $('<input type="file" accept="image/*" capture="user"/>')
	  }
      ...
      uploader.on('change', function () {
		...
		//  
         ...
         if (wrong == 0) reader.readAsDataURL(uploader[0].files[0])
...
var currentQuestion = urlChunks[urlChunks.length - 1].split('.');		  
		  ...
		  //console.log(toBeUploadedImg.type);
...
// Please do not forget to add an respondent identifier to the URLs of the respondents camera uploads for identifying respondents. 
...
// Please be aware that tic might be named differently in your survey (see above)
		...
		if (isReady == 0)
	  	{
			...
			if (currentUrl[0] == "https://localhost/SImage/Upload_Camera.html") window.location.href = "Upload_Camera.html?typ=" + "&tic=" + userID;
    ...
    function reset() {
	  ...
	  button.show();
	  deleteButton.hide();
	  document.getElementById("NotificationText").innerHTML = "<center>To take a photo, click on the camera icon.</center>";
	  document.getElementById("send").innerHTML = "Weiter";
	  	  isReady = 0;  
